...
public:
	...
	std::pair<bool, T> lower_bound(T const& e) const {
		return hayMayor(e, raiz);
	}

protected:

	std::pair<bool, T> hayMayor(T const& e, Link a) const {
		if (a == nullptr) return { false, T() };
		if (a->elem < e) return hayMayor(e, a->dr);
		if (e < a->elem) {
			auto izq = hayMayor(e, a->iz);
			if (izq.first) return izq;
			else return { true, a->elem };
		}
		//a->elem == e
		if (a->dr != nullptr) {
			auto actual = a->dr;
			while (actual->iz != nullptr) {
				actual = actual->iz;
			}
			return { true, actual->elem };
		}
		else return { false, T() };
	}
