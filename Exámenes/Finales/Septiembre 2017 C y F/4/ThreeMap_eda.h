...
public:
	...
	bool keyInBounds(Clave const & k) const {
		bool mayor = false, menor = false, encontrado = false;
		auto act = this->raiz;

		while (act != nullptr && (!mayor || !menor) && !encontrado) {
			if (this->menor(k, act->cv.clave)) {
				mayor = true;
				act = act->iz;
			}
			else if (this->menor(act->cv.clave, k)) {
				menor = true;
				act = act->dr;
			}
			else
...
#endif // treemap_eda_h#pragma once
