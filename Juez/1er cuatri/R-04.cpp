...
void resolverAux(vector<tJugador> const &jugadores, tJugador & ganador, int ini, int fin) {
	if (fin - ini == 2) { //Caso base, solo 2 jugadores
		...
		resolverAux(jugadores, ganadorIzq, ini, mitad);
		resolverAux(jugadores, ganadorDer, mitad, fin);
		ganador = quienGana(ganadorIzq, ganadorDer);
	}
}

void resolver(vector<tJugador> const &jugadores, tJugador & ganador) {
	resolverAux(jugadores, ganador, 0, jugadores.size());
}
